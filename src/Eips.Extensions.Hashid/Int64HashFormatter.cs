...
{
    /// <summary>
    /// 提供了计算 <see cref="long" /> 类型哈希格式化相关的方法。
    /// <para> 密闭的，不可以从此类型派生。 </para>
    /// </summary>
    /// <seealso cref="IHashIdFormatter{T}" />
    /// <seealso cref="HashIdFormatter{T}" />
    public sealed class Int64HashFormatter : HashIdFormatter<long>, IHashIdFormatter<long>
    {
        /// <summary> 用于初始化一个 <see cref="Int64HashFormatter" /> 类型的对象实例。 </summary>
        /// <param name="keyProvider"> 创建加密密钥的方法。 </param>
        public Int64HashFormatter(IHashIdCryptographyKeyProvider keyProvider) : base(keyProvider)
        {
        }

        /// <summary>
        /// 从哈希字符串 <paramref name="hashStr" /> 还原 <see cref="long" /> 类型的值。
        /// </summary>
        ...
        /// </exception>
        public override long? FromString(string hashStr)
        {
