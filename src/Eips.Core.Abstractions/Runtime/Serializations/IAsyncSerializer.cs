...
{
    /// <summary> 定义了异步的数据序列化接口。 </summary>
    /// <seealso cref="ISerializer" />
    public interface IAsyncSerializer : ISerializer
    {
        /// <summary>
        /// (异步的方法) 从流 <paramref name="destionationStream" /> 中反序列化得到
        ...
        Task<T> DeserializeAsync<T>(Stream destionationStream) where T : class, new();
        ...
        /// <returns> <typeparamref name="T" /> 类型的对象实例。 </returns>
        /// <seealso cref="Task{TResult}" />
        Task<T> DeserializeAsync<T>(byte[] input) where T : class, new();

        /// <summary>
        /// (异步的方法) 序列化 <typeparamref name="T" /> 类型的对象实例，并写入流 <paramref name="destionationStream" />。
        /// </summary>
        /// <typeparam name="T"> 引用类型。 </typeparam>
        ...
        /// <seealso cref="Stream" />
        /// <seealso cref="Task" />
        Task SerializeAsync<T>(T obj, Stream destionationStream) where T : class, new();
